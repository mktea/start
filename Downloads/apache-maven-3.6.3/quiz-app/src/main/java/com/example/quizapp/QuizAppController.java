...
public class QuizAppController {
    ...
    @GetMapping("/check")
    public String check(Model model, String question, boolean answer){
        for(Quiz quiz: quizzes){
            if(quiz.getQuestion().equals(question)){
                model.addAttribute("quiz",quiz);
                if(answer==quiz.isAnswer()) model.addAttribute("result","正解");
                else model.addAttribute("result","不正解");
            }
        }
        return "answer";
    }
    ...
    public String load(RedirectAttributes attributes){
        try {
            ...
            e.printStackTrace();
            attributes.addFlashAttribute("errorMessage","読み込みを失敗しました");
        }
        return "redirect:/page/show";
    }
}
